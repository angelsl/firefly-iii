 ...
 */
    ...
    'pref_two_factor_auth_help'                                   => '当您启用多因素认证（也称为双因素认证）时，可以为您的账户增加一层额外的安全保护。您使用您知道的（您的密码）和您有的（验证码）来进行登录。验证码由您手机上的应用程序如 Authy 或 Google 身份验证器生成。',
...
',
    ...
    'mfa_index_intro'                                             => 'Firefly III 支持多因素认证（MFA）。 您可以为您的帐户启用 MFA 来添加额外的一层安全保护。诸如 Authy、Google Authenticator 和 FreeOTP 之类的应用程序可以用于生成您需要登录的代码。 Firefly III 不支持安全密钥，但您可以使用安全密钥作为您的 MFA 密钥的存储设备。',
    ...
    'mfa_warning_code_changes'                                    => '您会在输入密码和 MFA 验证码后看到一个 MFA 对话框。在这种情况下，请等待您的应用程序生成一个新的 MFA 验证码，不要再循环使用您刚刚用过的验证码。',
    'mfa_already_disabled'                                        => '多因素认证未启用，所以您无法禁用它。',
    'disable_mfa_page'                                            => '禁用多因素认证',
    'disable_mfa_intro'                                           => '您可以禁用多因素认证，要禁用的话，请输入您的密码和多因素认证验证码。 如果您是因为失去了对代码生成器的访问权限，从而想要禁用多因素认证，请参考 <a href="https://docs.firefly-iii.org/references/faq/firefly-iii/using/#i-lost-my-2fa-token-generator-or-2fa-has-stopped-working">文档</a>。',
    'pref_disable_mfa'                                            => '禁用多因素认证',
    'mfa_not_enabled'                                             => '未启用多因素认证。',
    'mfa_backup_codes_intro'                                      => 'Firefly III 可以为您生成备用代码。当您不能使用验证代码生成器时，这些代码可以用于登录。 您可以随时生成一套新的代码。如果您生成新的备用代码，旧的备用代码将失效。',
    'mfa_backup_codes_quick'                                      => '如果您刚刚从多因素认证的设置页面跳转过来，您的应用可能还没有生成新的验证码。 请了解：MFA 验证码只能使用一次。请确保您使用不同于前一个的新验证码。',
